...
export default function AdminBannersScreen() {
  ...
  const pickImage = async () => {
    ...
    if (status !== "granted") {
      showAlert({
        ...
        message: "กรุณาอนุญาตให้เข้าถึงรูปภาพในตั้งค่า",
      });
      return;
    }
    ...
    try {
      ...
      if (base64Image) {
        ...
        const { error: uploadError } = await supabase.storage
          .from("avatars")
          .upload(`banners/${fileName}`, decode(base64Image), {
  ...
  const renderSlotCard = (slotDef: any) => {
    ...
    if (!existingData || !existingData.image_url) {
      return (
        <TouchableOpacity
          ...
          onPress={() => openModalForSlot(slotDef, null)}
        >
    ...
    const dateStr = existingData.created_at
      ? new Date(existingData.created_at).toLocaleDateString("th-TH", {
          day: "numeric",
          month: "short",
        })
      : "ล่าสุด";
    ...
    return (
      <View key={slotDef.id} style={styles.bannerCard}>
        <Image
          source={{ uri: existingData.image_url }}
          style={styles.bannerImage}
        />
        <View style={styles.bannerContent}>
          <Text style={styles.bannerTitle} numberOfLines={1}>
            {existingData.title || slotDef.name}
          </Text>
          ...
          <View style={styles.bannerMetaRow}>
            <View
              style={[
                ...
                {
                  backgroundColor:
                    existingData.is_active !== false ? "#EBE4FF" : "#F5F5F5",
                },
              ]}
            >
              <Text
                style={[
                  ...
                  {
                    color:
                      existingData.is_active !== false ? "#5E35B1" : "#9E9E9E",
                  },
                ]}
              >
          ...
          <View style={styles.cardActions}>
            <TouchableOpacity
              ...
              onPress={() => openModalForSlot(slotDef, existingData)}
            >
              <Text style={styles.editBtnText}>แก้ไข</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={styles.deleteBtn}
              onPress={() => handleDelete(existingData.id, slotDef.name)}
            >
        ...
        <View style={styles.headerTopRow}>
          ...
          <View style={{ flex: 1, marginLeft: 15 }}>
            ...
            <Text style={styles.headerSub}>
              เปลี่ยนรูปข่าวสารบนหน้า Home (ระบบ 5 ช่อง)
            </Text>
          </View>
        </View>
      </SafeAreaView>
        ...
        <View style={styles.warningBox}>
          ...
          <View style={{ marginLeft: 10, flex: 1 }}>
            ...
            <Text style={styles.warningDesc}>
              สไลเดอร์ระบบจะตัดแบบแนวนอน 16:9
            </Text>
            <Text style={styles.warningDesc}>
              ส่วนโปสเตอร์ด้านล่าง ระบบจะตัดเป็นแนวตั้ง 3:4
            </Text>
          </View>
        </View>
          ...
          <View style={styles.modalContent}>
            <ScrollView showsVerticalScrollIndicator={false}>
              <View style={styles.modalHeader}>
                <View>
                  <Text style={styles.modalTitle}>
                    {editingId ? "แก้ไขแบนเนอร์" : "ตั้งค่าแบนเนอร์ใหม่"}
                  </Text>
                  <Text style={styles.modalSubTitle}>
                    สำหรับช่อง: {activeSlot?.name}
                  </Text>
                </View>
              ...
              <TextInput
                ...
                placeholder="เช่น โปสเตอร์โปรโมชั่น"
                ...
                {previewImage ? (
                  ...
                  <View style={{ alignItems: "center" }}>
                    ...
                    <Text style={styles.dropzoneSub}>
                      ระบบจะบังคับให้ตัดภาพอัตโนมัติ
                    </Text>
                  </View>
                )}
              </TouchableOpacity>
              ...
              <View style={styles.rowGrid}>
                <TouchableOpacity
                  style={[
                    ...
                    isActive && styles.statusToggleActive,
                  ]}
                  ...
                  {uploading ? (
                    ...
                    <>
                      ...
                      <Text style={styles.saveBtnText}>
                        {editingId ? "บันทึกแก้ไข" : "บันทึกรูปภาพ"}
                      </Text>
                    </>
                  )}
                </TouchableOpacity>
