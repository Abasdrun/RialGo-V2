...
import { View, Text, StyleSheet, TouchableOpacity, TextInput, ActivityIndicator, ScrollView, KeyboardAvoidingView, Platform, Modal } from 'react-native';
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          if (!users || users.length === 0) {
            showAlert({ type: 'error', title: 'แจ้งเตือน', message: 'ไม่พบผู้ใช้งานในระบบ' });
            return;
          }
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <View style={[styles.modalIconCircle, 
              ...
              modalConfig.type === 'error' ? {backgroundColor: '#FFEBEE'} : {backgroundColor: '#FFF8E1'}
            ]}>
              <Ionicons 
                name={modalConfig.type === 'success' ? 'checkmark-circle' : modalConfig.type === 'error' ? 'alert-circle' : 'warning'} 
                size={40} 
                color={modalConfig.type === 'success' ? '#4CAF50' : modalConfig.type === 'error' ? '#EF5350' : '#FFB300'} 
              />
            </View>
            ...
            <View style={styles.modalActionRow}>
              ...
              <TouchableOpacity 
                style={[
                  ...
                  modalConfig.type === 'error' && { backgroundColor: '#EF5350' },
                  !modalConfig.showCancel ? { flex: 1, width: '100%' } : { flex: 1 }
                ]} 
...
const styles = StyleSheet.create({
  ...
  modalContainer: { width: '100%', backgroundColor: '#FFF', borderRadius: 25, padding: 25, alignItems: 'center', elevation: 10 },
  ...
  modalMessage: { fontSize: 14, color: '#666', textAlign: 'center', lineHeight: 22, marginBottom: 25 },
  modalActionRow: { flexDirection: 'row', width: '100%', gap: 10, justifyContent: 'center', alignItems: 'center' },
  modalCancelBtn: { flex: 1, paddingVertical: 14, borderRadius: 15, backgroundColor: '#F5F5F5', alignItems: 'center' },
  modalCancelText: { color: '#757575', fontWeight: 'bold' },
  modalConfirmBtn: { paddingVertical: 14, borderRadius: 15, backgroundColor: '#5E35B1', justifyContent: 'center', alignItems: 'center', minWidth: 120 },
