...
export default function AdminCouponsScreen() {
  ...
  const handleSaveCoupon = async () => {
    if (!title || !code || !expDate || !discountAmount) {
      ...
      showModernAlert('แจ้งเตือน', 'กรุณากรอกข้อมูลให้ครบ (ชื่อ, โค้ด, มูลค่า, วันหมดอายุ)');
      return;
    }
          ...
          {loading ? (
            ...
            <View style={styles.tbody}>
              {coupons.length > 0 ? coupons.map(item => (
                <React.Fragment key={item.id}>{renderCouponRow({item})}</React.Fragment>
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{paddingBottom: 20}}>
              ...
              <TextInput style={[styles.input, styles.textArea]} value={desc} onChangeText={setDesc} placeholder="อธิบายเงื่อนไขการใช้คูปอง..." multiline textAlignVertical="top" />
              <View style={styles.rowGrid}>
                <View style={styles.colHalf}><Text style={styles.label}>โค้ด (Code)</Text><TextInput style={styles.input} value={code} onChangeText={setCode} autoCapitalize="characters" /></View>
                <View style={styles.colHalf}><Text style={styles.label}>ประเภทส่วนลด</Text><View style={styles.dropdownFake}><Text>{discountType}</Text></View></View>
              </View>
              <View style={styles.rowGrid}>
                <View style={styles.colHalf}><Text style={styles.label}>ส่วนลด</Text><TextInput style={styles.input} value={discountAmount} onChangeText={setDiscountAmount} keyboardType="numeric" /></View>
                <View style={styles.colHalf}><Text style={styles.label}>ราคาขั้นต่ำ (THB)</Text><TextInput style={styles.input} value={minPrice} onChangeText={setMinPrice} keyboardType="numeric" /></View>
              </View>
              <View style={styles.rowGrid}>
                <View style={styles.colHalf}><Text style={styles.label}>จำกัดการใช้ (ครั้ง)</Text><TextInput style={styles.input} value={usageLimit} onChangeText={setUsageLimit} keyboardType="numeric" /></View>
                <View style={styles.colHalf}><Text style={styles.label}>วันหมดอายุ</Text><TextInput style={styles.input} value={expDate} onChangeText={setExpDate} placeholder="mm/dd/yyyy" /></View>
              </View>
              ...
              <View style={styles.modalActionRow}>
                <TouchableOpacity style={styles.cancelBtn} onPress={closeModal}><Text style={styles.cancelBtnText}>ยกเลิก</Text></TouchableOpacity>
                <TouchableOpacity style={styles.saveBtn} onPress={handleSaveCoupon}>
                  <Ionicons name="checkmark" size={16} color="#FFF" style={{marginRight: 5}}/><Text style={styles.saveBtnText}>{editingId ? 'บันทึกแก้ไข' : 'บันทึกคูปอง'}</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
          </View>
      ...
      <Modal visible={isDeleteModalVisible} animationType="fade" transparent>
        <View style={styles.deleteModalOverlay}>
          <View style={styles.deleteModalContent}>
            <View style={styles.warningIconBg}><Ionicons name="trash-outline" size={32} color="#F44336" /></View>
            <Text style={styles.deleteModalTitle}>ยืนยันการลบ</Text>
            <Text style={styles.deleteModalText}>คุณแน่ใจหรือไม่ว่าต้องการลบคูปองนี้?{"\n"}ข้อมูลที่ถูกลบจะไม่สามารถกู้คืนได้</Text>
            <View style={styles.deleteActionRow}>
              <TouchableOpacity style={styles.deleteCancelBtn} onPress={cancelDelete}><Text style={styles.deleteCancelBtnText}>ยกเลิก</Text></TouchableOpacity>
              <TouchableOpacity style={styles.deleteConfirmBtn} onPress={confirmDeleteCoupon}><Text style={styles.deleteConfirmBtnText}>ลบข้อมูล</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>
      ...
      <Modal visible={alertVisible} animationType="fade" transparent>
        <View style={styles.modernAlertOverlay}>
          <View style={styles.modernAlertContent}>
            <View style={[styles.modernAlertIconBg, { backgroundColor: alertTitle === 'สำเร็จ!' ? '#E8F5E9' : '#FFF8E1' }]}>
              <Ionicons 
            ...
            <TouchableOpacity 
              style={[styles.modernAlertBtn, { backgroundColor: alertTitle === 'สำเร็จ!' ? '#4CAF50' : '#5E35B1' }]} 
...
const styles = StyleSheet.create({
  ...
  headerBg: { backgroundColor: '#262956', borderBottomLeftRadius: 40, borderBottomRightRadius: 40, position: 'absolute', top: 0, left: 0, right: 0, height: 200, overflow: 'hidden' },
  ...
  // 🗑️ Delete Modal
  deleteModalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'center', alignItems: 'center', paddingHorizontal: 20 },
  deleteModalContent: { backgroundColor: '#FFF', width: '100%', borderRadius: 24, padding: 24, alignItems: 'center', elevation: 10 },
  warningIconBg: { width: 64, height: 64, borderRadius: 32, backgroundColor: '#FFEBEE', justifyContent: 'center', alignItems: 'center', marginBottom: 16 },
  deleteModalTitle: { fontSize: 18, fontWeight: 'bold', color: '#333', marginBottom: 8 },
  deleteModalText: { fontSize: 13, color: '#757575', textAlign: 'center', marginBottom: 24, lineHeight: 20 },
  deleteActionRow: { flexDirection: 'row', width: '100%', justifyContent: 'space-between' },
  deleteCancelBtn: { flex: 1, paddingVertical: 14, borderRadius: 14, backgroundColor: '#F5F5F5', marginRight: 8, alignItems: 'center' },
  deleteCancelBtnText: { color: '#757575', fontSize: 14, fontWeight: 'bold' },
  deleteConfirmBtn: { flex: 1, paddingVertical: 14, borderRadius: 14, backgroundColor: '#F44336', marginLeft: 8, alignItems: 'center', elevation: 2 },
  deleteConfirmBtnText: { color: '#FFF', fontSize: 14, fontWeight: 'bold' },

  // 🟡 NEW: Modern Alert Modal Styles
  modernAlertOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'center', alignItems: 'center', paddingHorizontal: 40 },
  modernAlertContent: { backgroundColor: '#FFF', width: '100%', borderRadius: 30, padding: 25, alignItems: 'center', elevation: 20 },
  modernAlertIconBg: { width: 80, height: 80, borderRadius: 40, justifyContent: 'center', alignItems: 'center', marginBottom: 20 },
  modernAlertTitle: { fontSize: 20, fontWeight: 'bold', color: '#333', marginBottom: 10 },
  modernAlertText: { fontSize: 14, color: '#666', textAlign: 'center', marginBottom: 30, lineHeight: 22 },
  modernAlertBtn: { width: '100%', paddingVertical: 15, borderRadius: 15, alignItems: 'center', shadowOpacity: 0.2 },
