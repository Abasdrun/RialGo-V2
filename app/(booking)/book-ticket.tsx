...
export default function BookTicketScreen() {
  // ==========================================
  // 1. การประกาศตัวแปร State (ข้อมูลภายในหน้าจอ)
  // ==========================================
  const [tripType, setTripType] = useState<"one-way" | "round-trip">("one-way"); // ประเภทการเดินทาง (เที่ยวเดียว/ไปกลับ)
  ...
  const [passengers, setPassengers] = useState({
    ...
    child: 0,
    infant: 0,
  }); // จำนวนผู้โดยสารแต่ละประเภท

  const [trainType, setTrainType] = useState("รถด่วนพิเศษ"); // ประเภทขบวนรถ
  const [cabinClass, setCabinClass] = useState("ตู้นอนปรับอากาศ ชั้น 2"); // ชั้น/ประเภทตู้
  const [cabinNumber, setCabinNumber] = useState("1"); // เลขตู้ขบวน

  const [stations, setStations] = useState<any[]>([]); // ข้อมูลสถานีทั้งหมด
  const [searchQuery, setSearchQuery] = useState(""); // ข้อความค้นหาสถานี
  const [loadingStations, setLoadingStations] = useState(false); // สถานะการโหลด
  const [selectingType, setSelectingType] = useState<"origin" | "destination">(
    "origin",
  ); // กำลังเลือกต้นทางหรือปลายทาง

  const [activeModal, setActiveModal] = useState<string | null>(null); // ควบคุมการเปิด Modal ต่างๆ

  const [alertConfig, setAlertConfig] = useState({
    visible: false,
    title: "",
    message: "",
  }); // การตั้งค่า Alert แจ้งเตือน

  const [depMonthOffset, setDepMonthOffset] = useState(0); // การเลื่อนเดือนในปฏิทินวันไป
  const [retMonthOffset, setRetMonthOffset] = useState(0); // การเลื่อนเดือนในปฏิทินวันกลับ

  const [recentStations, setRecentStations] = useState<any[]>([]); // สถานีที่เลือกบ่อย
  const [selectedRegion, setSelectedRegion] = useState("ทั้งหมด"); // ตัวกรองภูมิภาค

  const [recentSearches, setRecentSearches] = useState<any[]>([]); // ประวัติการค้นหาล่าสุด

  // ==========================================
  // 2. ฟังก์ชันจัดการข้อมูล (Logic & Functions)
  // ==========================================

  // ดึงข้อมูลสถานีเมื่อเปิดหน้าจอ
  useEffect(() => {
    fetchStations();
  }, []);
  ...
  // รายการตัวเลือกประเภทตู้ตามประเภทรถ
  const getCabinOptions = () => {
    return trainType === "รถด่วนพิเศษ"
      ? [
          "ตู้นอนปรับอากาศ ชั้น 2",
          "ตู้นอนปรับอากาศ ชั้น 2 สำหรับผู้พิการ/วีลแชร์",
          "ตู้นอนปรับอากาศ ชั้น 1",
        ]
      : [
          "ตู้นั่งพัดลม ชั้น 3",
          "ตู้นั่งปรับอากาศ / พัดลม ชั้น 2",
          "ตู้นอนปรับอากาศ ชั้น 2",
        ];
  };

  // กำหนดเลขตู้ที่เลือกได้ตามเงื่อนไขประเภทรถและตู้
  ...
  // ตรวจสอบความถูกต้องของเลขตู้เมื่อมีการเปลี่ยนประเภทรถหรือชั้น
  ...
  // คำนวณข้อมูลวันที่สำหรับแสดงในปฏิทิน
  ...
  // กำหนดสีและไอคอนตามข้อความในเมนูเลือก
  const getPopupIconStyle = (text: string, isNumber = false) => {
    if (isNumber)
      return { bg: "#F9E8B6", color: "#FBC02D", icon: "train-outline" };
    if (text.includes("ด่วนพิเศษ") || text.includes("ชั้น 1"))
      return { bg: "#EBE4FF", color: "#5E35B1", icon: "train" };
    if (text.includes("รถเร็ว") || text.includes("พัดลม"))
      return { bg: "#D4F1E5", color: "#4CAF50", icon: "train" };
    if (text.includes("ปรับอากาศ"))
      return { bg: "#D4F1E5", color: "#4CAF50", icon: "train" };
    return { bg: "#F5F5F5", color: "#757575", icon: "train" };
  };

  // กำหนดสไตล์และ Badge ตามภูมิภาคของสถานี
  const getStationStyle = (item: any) => {
    ...
    if (region === "อีสาน")
      return {
        bg: "#EDE7F6",
        iconColor: "#5E35B1",
        badgeText: "สายตะวันออกเฉียงเหนือ",
        badgeBg: "#D1C4E9",
        badgeTextColor: "#5E35B1",
      };
    if (region === "กลาง")
      return {
        bg: "#FFF3E0",
    ...
    if (region === "ตะวันตก")
      return {
        bg: "#FBE9E7",
        iconColor: "#FF5722",
        badgeText: "สายตะวันตก",
        badgeBg: "#FFEBEE",
        badgeTextColor: "#FF5722",
      };
    return {
      bg: "#D1C4E9",
      iconColor: "#5E35B1",
  ...
  // ตรวจสอบความถูกต้องและส่งข้อมูลไปหน้าค้นหา
  const handleSearchSubmit = () => {
    if (origin.includes("เลือก") || destination.includes("เลือก")) {
      showAlert(
        "กรอกข้อมูลไม่ครบ",
        "กรุณาเลือกสถานีต้นทาง และสถานีปลายทางให้เรียบร้อยครับ",
      );
      return;
    }
    ...
    if (tripType === "round-trip" && returnDate.includes("เลือก")) {
      showAlert(
        "กรอกข้อมูลไม่ครบ",
        "คุณเลือกแบบไป-กลับ กรุณาระบุวันกลับให้เรียบร้อยครับ",
      );
      return;
    }
  ...
  // ==========================================
  // 3. ส่วนการ Render UI (คอมโพเนนต์ย่อย)
  // ==========================================

  // แสดงผลส่วนปฏิทินใน Modal
        ...
        <View style={styles.calendarHeaderRow}>
          <View>
            ...
            <Text style={styles.calendarSubText}>
              {isRet ? "เลือกวันกลับ" : "เลือกวันไป"}
            </Text>
          </View>
          <View style={styles.calendarArrows}>
            <TouchableOpacity
              onPress={() => setOffset(offset - 1)}
              style={styles.calArrowBox}
            >
        ...
        >
          {["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"].map((d, idx) => (
            <Text
              ...
              style={[
                styles.dayHead,
                (idx === 0 || idx === 6) && { color: "#E91E63" },
              ]}
            >
              {d}
            </Text>
          ))}
          {monthData.empty.map((i) => (
  ...
  // คัดกรองสถานีตามการค้นหาและภูมิภาค
  const filteredAllStations = stations.filter((s) => {
    const matchSearch =
      s.station_name.includes(searchQuery) ||
      (s.province && s.province.includes(searchQuery));
    const matchRegion =
      selectedRegion === "ทั้งหมด" || s.region === selectedRegion;
    return matchSearch && matchRegion;
  });

  // ==========================================
  // 4. ส่วนแสดงผลหน้าจอหลัก (Main JSX)
  // ==========================================
  return (
    <View style={styles.mainContainer}>
          ...
          <View style={styles.tabContainer}>
            ...
            >
              <Text
                style={[
                  styles.tabText,
                  tripType === "one-way" && styles.tabTextActive,
                ]}
              >
                เที่ยวเดียว
              </Text>
            ...
            <TouchableOpacity
              style={[
                styles.tab,
                tripType === "round-trip" && styles.tabActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.tabText,
                  tripType === "round-trip" && styles.tabTextActive,
                ]}
              >
                ไปกลับ
              </Text>
      ...
      >
            ...
            <View style={{ flex: 1 }}>
              ...
              >
                ...
                <Text
                  style={[
                    styles.stationValue,
                    origin.includes("เลือก") && { color: "#9E9E9E" },
                  ]}
                >
                  {origin}
                </Text>
              ...
              >
                ...
                <Text
                  style={[
                    styles.stationValue,
                    destination.includes("เลือก") && { color: "#9E9E9E" },
                  ]}
                >
                  {destination}
                </Text>
          ...
          <View style={styles.dateRow}>
            ...
            >
              ...
              <Text
                style={[
                  styles.dateValue,
                  departureDate.includes("เลือก") && { color: "#9E9E9E" },
                ]}
              >
                {departureDate}
              </Text>
            ...
            <TouchableOpacity
              style={[
                styles.dateBox,
                tripType === "one-way" && styles.dateBoxDisabled,
              ]}
              onPress={() => setActiveModal("calendar")}
            ...
            >
              <View style={styles.dateTitleRow}>
                <Ionicons
                  ...
                  size={14}
                  color={tripType === "one-way" ? "#BDBDBD" : "#757575"}
                />
                <Text
                  style={[
                    styles.inputLabelSmall,
                    tripType === "one-way" && { color: "#BDBDBD" },
                  ]}
                >
                  วันกลับ
                </Text>
              ...
              <Text
                style={[
                  styles.dateValue,
                  (returnDate.includes("เลือก") || tripType === "one-way") && {
                    color: "#BDBDBD",
                  },
                ]}
              >
        ...
        {/* การ์ดเลือกจำนวนผู้โดยสาร */}
        <View style={styles.card}>
          <Text style={styles.cardTitle}>จำนวนผู้โดยสาร</Text>
          <View style={styles.passengerRow}>
            <View style={[styles.iconBox, { backgroundColor: "#EBE4FF" }]}>
              <Ionicons name="person" size={20} color="#5E35B1" />
            </View>
            ...
            <View style={styles.counterGroup}>
              <TouchableOpacity
                ...
                onPress={() => updatePassenger("adult", "sub")}
              >
              ...
              <TouchableOpacity
                style={styles.countBtn}
                onPress={() => updatePassenger("adult", "add")}
              >
                <Text style={styles.countBtnText}>+</Text>
              </TouchableOpacity>
            </View>
          ...
          <View style={styles.passengerRow}>
            <View style={[styles.iconBox, { backgroundColor: "#D4F1E5" }]}>
            ...
            <View style={styles.counterGroup}>
              <TouchableOpacity
                style={styles.countBtn}
                onPress={() => updatePassenger("child", "sub")}
              >
                <Text style={styles.countBtnText}>−</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                style={styles.countBtn}
                onPress={() => updatePassenger("child", "add")}
              >
                <Text style={styles.countBtnText}>+</Text>
              </TouchableOpacity>
            </View>
          ...
          >
            <View style={[styles.iconBox, { backgroundColor: "#F9E8B6" }]}>
            ...
            <View style={styles.counterGroup}>
              <TouchableOpacity
                style={styles.countBtn}
                onPress={() => updatePassenger("infant", "sub")}
              >
                <Text style={styles.countBtnText}>−</Text>
              </TouchableOpacity>
              ...
              <TouchableOpacity
                style={styles.countBtn}
                onPress={() => updatePassenger("infant", "add")}
              >
                <Text style={styles.countBtnText}>+</Text>
              </TouchableOpacity>
            </View>
        ...
        {/* การ์ดเลือกรายละเอียดขบวนรถ */}
        <View style={styles.card}>
          <Text style={styles.cardTitle}>ชั้นและตู้โดยสาร</Text>
          <TouchableOpacity
            style={styles.optionRow}
            onPress={() => setActiveModal("trainType")}
          >
            <View style={[styles.iconBox, { backgroundColor: "#EBE4FF" }]}>
          ...
          <TouchableOpacity
            style={styles.optionRow}
            onPress={() => setActiveModal("cabinClass")}
          >
            <View style={[styles.iconBox, { backgroundColor: "#D4F1E5" }]}>
              <MaterialCommunityIcons
          ...
          <TouchableOpacity
            ...
            onPress={() => setActiveModal("cabinNumber")}
          >
            <View style={[styles.iconBox, { backgroundColor: "#F9E8B6" }]}>
              <Ionicons name="train-outline" size={20} color="#FBC02D" />
            </View>
            <View style={styles.optionInfo}>
          ...
          <View style={styles.recentSection}>
            <View style={styles.recentHeader}>
              ...
              <TouchableOpacity onPress={() => setRecentSearches([])}>
                <Text style={styles.seeAllText}>ล้างประวัติ</Text>
              </TouchableOpacity>
            </View>
            <ScrollView
              horizontal
            ...
            >
              {recentSearches.map((item, index) => (
                ...
                >
                  <Text style={styles.recentCityText} numberOfLines={1}>
                    {item.origin}
                  </Text>
                  <Ionicons
                    name="arrow-down"
      ...
      <Modal visible={activeModal === "station"} animationType="slide">
        <View style={styles.mainContainer}>
          ...
          <ScrollView
            contentContainerStyle={styles.stationModalScroll}
            showsVerticalScrollIndicator={false}
          >
                ...
                {regionFilters.map((r) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      selectedRegion === r.id && styles.regionFilterChipActive,
                    ]}
                ...
                <View style={styles.recentStationSection}>
                  ...
                  {recentStations.map((item, index) => {
                    ...
                    return (
                      <TouchableOpacity
                        key={`recent-${index}`}
                        style={styles.recentStationItem}
                      ...
                      >
                        ...
                        <View style={styles.stationTextWrapper}>
                          ...
                          <Text style={styles.stationSubText}>
                            {item.province}{" "}
                            {sStyle.badgeText ? `• ${sStyle.badgeText}` : ""}
                          </Text>
                        </View>
                      </TouchableOpacity>
                    );
            ...
            <Text
              style={[
                ...
                {
                  marginTop:
                    searchQuery || selectedRegion !== "ทั้งหมด" ? 0 : 10,
                },
              ]}
            >
              {searchQuery || selectedRegion !== "ทั้งหมด"
                ? "ผลการค้นหา"
                : "สถานีทั้งหมด"}
            </Text>
                  ...
                  >
                    ...
                    {sStyle.badgeText ? (
                      <View
                        style={[
                          styles.lineBadge,
                          { backgroundColor: sStyle.badgeBg },
                        ]}
                      >
                        <Text
                          style={[
                            styles.lineBadgeText,
                            { color: sStyle.badgeTextColor },
                          ]}
                        >
                          {sStyle.badgeText}
                        </Text>
      ...
      {/* Bottom Sheet Modal สำหรับเลือกประเภทรถและตู้ */}
      <Modal
        visible={activeModal === "trainType" || activeModal === "cabinClass"}
        transparent
        animationType="slide"
      >
        <View style={styles.sheetOverlay}>
          ...
          <View style={styles.whiteSheet}>
            ...
            <Text style={styles.sheetTitleLeft}>
              {activeModal === "trainType" ? "ชั้นโดยสาร" : "ประเภทตู้โดยสาร"}
            </Text>
            <ScrollView showsVerticalScrollIndicator={false}>
              {(activeModal === "trainType"
                ? ["รถเร็ว", "รถด่วนพิเศษ"]
                : getCabinOptions()
              ).map((opt) => {
                const isSelected =
      ...
      {/* Modal เลือกเลขตู้ขบวน */}
      <Modal
        visible={activeModal === "cabinNumber"}
        transparent
        animationType="slide"
      >
        <View style={styles.sheetOverlay}>
